...
export const MonthlyActivityGrid = ({
  ...
  return <div className="backdrop-blur-xl bg-white/5 dark:bg-white/80 rounded-2xl border border-white/10 dark:border-gray-200 p-6 shadow-lg">
      {/* Header with navigation */}
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center space-x-4">
          <Button variant="ghost" size="sm" onClick={() => navigateMonth('prev')} className="text-gray-300 dark:text-gray-700 hover:text-white dark:hover:text-black hover:bg-white/10 dark:hover:bg-gray-200">
          ...
          <Button variant="ghost" size="sm" onClick={() => navigateMonth('next')} className="text-gray-300 dark:text-gray-700 hover:text-white dark:hover:text-black hover:bg-white/10 dark:hover:bg-gray-200">
      ...
      <div className="space-y-3">
        ...
        <div className="flex items-center">
          ...
          <div className="flex gap-1 ml-4">
            {days.map(day => <div key={day} className="w-4 h-4 flex items-center justify-center text-[10px] text-gray-500 dark:text-gray-600 font-mono" title={`Day ${day}`}>
                {day}
              </div>)}
          </div>
        </div>
        ...
        return <div key={habit.id} className="flex items-center">
              <div className="w-40 text-sm text-gray-300 dark:text-gray-800 truncate pr-3">
                {habit.name}
              </div>
              <div className="flex gap-1 ml-4">
              ...
              return <div key={day} className={`w-4 h-4 ${isCompleted ? 'bg-green-600 dark:bg-green-500' : 'bg-gray-800 dark:bg-gray-300 border border-gray-700 dark:border-gray-400'}`} title={`${habit.name} - Day ${day} (${isCompleted ? 'Completed' : 'Not completed'})`} />;
            })}
              </div>
        ...
        {habits.length === 0 && <div className="text-center py-8 text-gray-400 dark:text-gray-600">
